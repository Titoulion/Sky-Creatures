...
////// Emissive:
                ...
                float node_9670 = ((cos(_TransitionColor)+1.0)*0.5);
                float node_5998 = 0.0;
                float3 node_3404 = lerp(_RampA_var.rgb,_RampB_var.rgb,saturate((floor(_TextureColorTransition_var.r * _posterizeTransition) / (_posterizeTransition - 1)+(_remapTransitionMin + ( (node_9670 - node_5998) * (_remapTransitionMax - _remapTransitionMin) ) / (1.0 - node_5998)))));
